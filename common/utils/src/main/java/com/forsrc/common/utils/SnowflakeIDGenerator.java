 ...
 * https://github.com/twitter/snowflake). Snowflake IDs are 64 bit positive
 * longs composed of: - 41 bits time stamp - 10 bits machine id - 12 bits
 * sequence number
 *
 *
...
public class SnowflakeIDGenerator /* implements IDGenerator */ {
    ...
    public SnowflakeIDGenerator(long datacenterId) {
        if (datacenterId == 0) {
            try {
                ...
                // log.warn("SNOWFLAKE: could not determine machine address; using random
                // datacenter ID");
        ...
        if (this.datacenterId > maxDatacenterId || datacenterId < 0) {
            // log.warn("SNOWFLAKE: datacenterId > maxDatacenterId; using random datacenter
            // ID");
            Random rnd = new Random();
            this.datacenterId = rnd.nextInt((int) maxDatacenterId) + 1;
        }
    ...
    public synchronized long getId() {
        ...
        if (timestamp < lastTimestamp) {
            // log.warn("Clock moved backwards. Refusing to generate id for {}
