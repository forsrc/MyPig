...
@RestController
@RequestMapping("/api/v2/tcc/user")
public class UserTccReactiveController implements UserTccFeignClient {

    @Autowired
    private UserTccService userTccService;

    private static final Logger LOGGER = LoggerFactory.getLogger(UserTccReactiveController.class);
    ...
    public ResponseEntity<UserTcc> tccTry(@RequestBody UserTcc tcc, @RequestHeader("Authorization") String accessToken)
        ...
        return ResponseEntity.status(HttpStatus.CREATED).header("tccLinkId", tcc.getId().toString())
                .header("tccLinkStatus", String.valueOf(userTcc.getStatus())).body(userTcc);
    }

    @PreAuthorize("hasRole('ADMIN') or hasRole('TCC')")
    @PutMapping(path = "/confirm/{id}")
    @HystrixCommand()
    public ResponseEntity<Void> confirm(@PathVariable("id") Long id, @RequestHeader("Authorization") String accessToken)
            throws TccException {
        ...
        return ResponseEntity.noContent().header("tccLinkId", String.valueOf(id))
                .header("tccLinkStatus", String.valueOf(userTcc.getStatus())).build();
    }

    @PreAuthorize("hasRole('ADMIN') or hasRole('TCC')")
    @DeleteMapping(path = "/cancel/{id}")
    @HystrixCommand()
    public ResponseEntity<Void> cancel(@PathVariable("id") Long id, @RequestHeader("Authorization") String accessToken)
            throws TccException {
