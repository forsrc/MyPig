...
INSERT INTO t_tcc_link (id, tcc_id, uri, path, expire, status) SELECT * FROM (SELECT  uuid_in('36ef1f69e4c942f7ab55f1c77231c582') id, uuid_in('d4e55207db0a4b8e969190305cb51a44') tcc_id, 'http://SPRINGBOOT-SSO-SERVER/sso/api/v1/tcc/user/' uri, '5112cdecfec04f2e8d364ed1d14b7882' entity_id, (CURRENT_TIMESTAMP + '5 minute') expire, 0 status) AS T WHERE NOT EXISTS (SELECT id FROM t_tcc_link WHERE id = '36ef1f69e4c942f7ab55f1c77231c582');
