...
INSERT INTO t_tcc_link (id, tcc_id, uri, resource_id, expire, status) SELECT * FROM (SELECT  1 id, 1 tcc_id, 'http://SPRINGBOOT-SSO-SERVER/sso/api/v1/tcc/user/' uri, 1 resource_id, TIMESTAMPADD(MINUTE, 5, CURRENT_TIMESTAMP) expire, 0 status) AS T WHERE NOT EXISTS (SELECT id FROM t_tcc_link WHERE id = 1);
